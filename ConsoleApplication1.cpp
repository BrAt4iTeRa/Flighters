...
flights* creator(flights* p, int& ñounter) {
	int end = 1;
	system("CLS");
	printf_s("Hello. It's creator:\n");
	for (; end != 0; ñounter++) {
		p = (flights*)realloc(p, sizeof(flights) * (ñounter + 1));
		printf_s("It's a flight ¹%d. Enter information, following instructions.\nEnter nomber of flight:\n", ñounter + 1);
		scanfMod(p[ñounter].number);
		printf_s("Enter destination:\n");
		rewind(stdin);
		fgets(p[ñounter].destination, 10, stdin);
		for (int j = 0; p[ñounter].destination; j++) {
			if (p[ñounter].destination[j] == '\n') {
				p[ñounter].destination[j] = '\0';
				break;
			}
		}
		printf_s("Enter flight time.\n\tEnter hour:\n\t");
		do {
			scanfMod(p[ñounter].time.hour);
			if (p[ñounter].time.hour > 23 || p[ñounter].time.hour < 0) {
				printf_s("Error.\n");//äîïèñàòü
			}
			else break;
		} while (1);
		...
		do {
			scanfMod(p[ñounter].time.minutes);
			if (p[ñounter].time.minutes > 59 || p[ñounter].time.minutes < 0) {
				printf_s("Error.\n");//äîïèñàòü
			}
			else break;
		} while (1);
		/*if (((*p)[ñounter].time.minutes / 10) == 0);*/
		printf_s("Enter date of flight.\n\t Enter year:\n\t");
		scanfMod(p[ñounter].date.year);
		printf_s("\tEnter month:\n\t");
		do {
			if (!(scanf_s("%d", &p[ñounter].date.month))) {
		...
		printf_s("\tEnter day:\n\t");
		do {
			if (!(scanf_s("%d", &p[ñounter].date.day))) {
				printf_s("Error! Try again:\n");
				rewind(stdin);
			}
			else break;
		...
		scanfMod(end);
		system("cls");
		//p = (flights*)realloc(p, sizeof(flights) * (ñounter);
	}
	system("cls");
	printf_s("=======================================================\n");
...
void correctDatabase(flights* p, int amount) {
	...
	scanf_s("%d", &number);//èíòåðåñíàÿ ïðîâåðêà ñäåëàé ÷åðåç ñàéçîô**
...
void deleteStruct(flights* &p, int &amount) {// êàê 
	int number, counter;
	system("CLS");
	printf_s("Enter the flight number you want to delete:\n");
	scanf_s("%d", &number);//èíòåðåñíàÿ ïðîâåðêó ñäåëàé ÷åðåç ñàéçîô
	for (counter = 0; counter < amount; counter++)
		if (p[counter].number == number) break;
	if (counter == amount) {
		printf_s("There is no such number. Check that the data you entered is correct and repeat again.");
